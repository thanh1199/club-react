...
function ShowNextDay ({ today, tomorrow }) {
    const nextday = today.getHours()>9 ? tomorrow : today
    ...
    const nextdayLessons = lessons[(nextday.getDay()-1)%5]
    return (<>
    {
        Object.keys(nextdayLessons).map((p, i1) => {
            return (<div key={i1}>
                <>{i1 < 1 ? `次のスケジュールは ${nextday.getMonth()+1}月${nextday.getDate()}日` : ""}</>
